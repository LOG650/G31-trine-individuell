# Vedlegg 2: Simuleringskode (Python)
...
def demand_for_day() -> dict:
    """Generate daily demand for all products."""
    result = {}
    for product in PRODUCTS:
        if random.random() < 0.2:  # 20% campaign days
            lam = 8.0
        else:
            lam = 3.0
...
def lead_time_days() -> int:
    """Generate realistic lead time: 15% 1 day, 65% 2 days, 20% 3 days."""
    r = random.random()
    if r < 0.15:
        return 1
    if r < 0.8:  # 0.15 + 0.65 = 0.8
        return 2
    return 3


def adjust_for_tuesday(arrival_day: int) -> int:
    """Adjust delivery day if it falls on Tuesday (no deliveries)."""
...
def next_multiple_of_3(value: int) -> int:
    """Round up to next multiple of 3 for ordering."""
...
def simulate(variant_name: str, variant: dict, daily_demands: list, days: int = DAYS) -> dict:
    """Run simulation for one variant."""
...
def generate_demand_series(days: int = DAYS) -> list:
    """Generate demand series for all days."""
